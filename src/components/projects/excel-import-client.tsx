...
async function parseImportResponse(response: Response) {
  ...
  if (!contentType.includes("application/json")) {
    return { importedCount: 0, duplicateCount: 0, message: rawBody };
  }

  try {
    return JSON.parse(rawBody) as { error?: string; importedCount?: number; duplicateCount?: number; message?: string };
  } catch {
    throw new Error(`Import failed: backend returned invalid JSON (${contentType || "unknown content-type"})`);
  }
}
...
export function ExcelImportClient({ projectId }: Props) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      setMessage(
        result.message ??
          `Imported ${result.importedCount ?? 0} items. Duplicates flagged: ${result.duplicateCount ?? 0}.`,
      );
    } catch (importError) {
